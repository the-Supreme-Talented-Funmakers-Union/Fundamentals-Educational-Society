...
using TMPro;
using UnityEngine;
public class GameplayM : MonoBehaviour
{
    public ButtonM button;
    public DealerM dealer;
    ...
    public IEnumerator ConfirmSelection()
    {
        if (!newGoal)
        {
            if (Card1.childCount == 0 || Card2.childCount == 0 || Card3.childCount == 0)
            {
                yield break;
            }
            float result = 0f;
            ...
            int score = (int)value1 + (int)value2 + (int)value3;
